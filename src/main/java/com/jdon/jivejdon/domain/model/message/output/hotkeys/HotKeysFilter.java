...
public class HotKeysFilter implements Function<MessageVO, MessageVO> {
	...
	public static void main(String[] args) {
		...
		// Example usage
		String largeText = "啊实打实banq注 s注 wordjpg   模在软件工程领域的成就方面品设计的方法/程序，涉及集成或组合较小的独立元素以WORD创建成品。大型产品（如汽车）可以分为更小、更简单的组件，并使用模块化设计方法单独开发和生产。最终产品是通过集成（或组装）这些组件中的banq注每一个来创建的。";
	...
	private String convertSearch(ConcurrentMap<String, String> searchMap, String chunk) {
		try {
			for (String key : searchMap.keySet()) {
				...
				if (m.find())
					m.appendReplacement(result, getKeyUrlStr(key, searchMap.remove(key)));
				m.appendTail(result);
