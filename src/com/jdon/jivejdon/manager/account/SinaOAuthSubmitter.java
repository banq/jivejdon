...
public class SinaOAuthSubmitter {
	...
	public String authorizeURL(String backurl, String response_type) {
		return oAuthParamVO.authorizeURL + "?client_id=" + oAuthParamVO.CONSUMER_KEY + "&redirect_uri=" + backurl + "&response_type=" + response_type;

	}

	public AccessToken getAccessTokenByCode(String code, String backurl) throws WeiboException {
		return new AccessToken(weibo.client.post(oAuthParamVO.accessTokenURL, new PostParameter[] {
				new PostParameter("client_id", oAuthParamVO.CONSUMER_KEY), new PostParameter("client_secret", oAuthParamVO.CONSUMER_SECRET),
				new PostParameter("grant_type", "authorization_code"), new PostParameter("code", code), new PostParameter("redirect_uri", backurl) },
				false));
	}
	...
	public void update(AccessToken access, String content) throws Exception {
		try {
			...
			Status status = new Status(weibo.client.post(oAuthParamVO.baseURL + "statuses/share.json",
	...
	public OAuthUserVO getUserInfo(AccessToken access) throws Exception {
		...
		try {
			Weibo weibo = new Weibo();
			weibo.setToken(access.getAccessToken());
			User user = new User(weibo.client.get(oAuthParamVO.baseURL + "users/show.json",
					new PostParameter[] { new PostParameter("uid", access.getUid()) }).asJSONObject());
			...
			weiboUser = new OAuthUserVO(userId, atName, user.getDescription(), userDomainURL, profileUrl);
		} catch (Exception e) {
			throw new Exception(e);
		}
		return weiboUser;
