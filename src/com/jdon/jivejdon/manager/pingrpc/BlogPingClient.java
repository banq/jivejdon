...
public class BlogPingClient {
	...
	public BlogPingClient(BlogPingTempParams blogPingTempParams, BlogPingServerUrl blogPingServerUrl, ThreadTimer threadTimer) {
	...
	class BlogPingSender extends Thread {
		...
		public void sendPing(Notification notification, String blogPingServerUrl) {
			...
			String title = StringUtil.replace(blogPingTempParams.getTitle(), "#username#", message.getUsername());
			String blogUrl = StringUtil.replace(blogPingTempParams.getBlogUrl(), "#username#", message.getUsername());
			String UrlChanges = StringUtil.replace(blogPingTempParams.getUrlChanges(), "#threadId#", Long.toString(message.getThreadId()));
			String urlRSS = StringUtil.replace(blogPingTempParams.getUrlRSS(), "#username#", message.getUsername());
			logger.info(blogPingServerUrl + " title=" + title + " blogUrl=" + blogUrl + " UrlChanges=" + UrlChanges + " urlRSS" + urlRSS);
			blogPing(blogPingServerUrl, title, blogUrl, UrlChanges, urlRSS);
		}

		public void blogPing(String serverUrl, String Title, String blogUrl, String UrlChanges, String UrlRSS) {
			try {
