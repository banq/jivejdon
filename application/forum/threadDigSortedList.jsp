...
<link rel="canonical" href="<%=domainUrl%>/threadDigSortedList/">  
  
   <%if(pagestartInt != 0 ) {%> 
        <%if(pagestartInt-pagecountInt>0 ) {%>  
            <link rel="prev" href="<%=domainUrl%>/threadDigSortedList/<%=(pagestartInt-pagecountInt)%>"/>
            <meta name="robots" content="noindex, nofollow">
        <%}else{%>
            <link rel="prev" href="<%=domainUrl%>/threadDigSortedList/"/>
            <meta name="robots" content="noindex, nofollow">
         <%}%>
        <%}%>
...
<div class="col-lg-4">
 <%if(pagestartInt != 0 ) {%> 
  <ul class="pagination pull-left">
    <li>
    <%if(pagestartInt-pagecountInt>0 ) {%>  
        <a href="<%=domainUrl%>/threadDigSortedList/<%=(pagestartInt-pagecountInt)%>" rel="prev nofollow" class="btn-page">上页</a>
    <%}else{%>
        <a href="<%=domainUrl%>/threadDigSortedList/" rel="prev" class="btn-page">上页</a>
     <%}%>
    </li>
  </ul>
 <%}%>
...
<div class="col-lg-4">
  <ul class="pagination pull-right"> 
    <li>
    <%if((pagestartInt+pagecountInt) < pageAllcountInt ) {%> 
    <a href="<%=domainUrl%>/threadDigSortedList/<%=pagestartInt+pagecountInt%>" rel="next nofollow" class="btn-page">下页</a>
    <%}%>
  </li>  
</ul>
</div>
...
<div class="box" style="border-radius: 12px; box-shadow: 0px 5px 15px rgba(150, 1, 1, 0.1); background-color: white; overflow: hidden; padding-left: 0; padding-right: 0">
        <!---- Start Widget ---->
