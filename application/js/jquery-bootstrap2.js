...
function subLoging() {
  ...
  $.ajax({
    url:
      "/account/protected/logged.jsp?j_username=" +
      j_username +
      "&j_password=" +
      j_password,
    success: function (response) {
      if (response.indexOf("if(setLogged)setLogged()") >= 0) setLogged();
      else alert("登录错误 ");
      if (logged)
...
function createCookie(name, value, seconds) {
  var dt = new Date();
  if (seconds) dt.setTime(dt.getTime() + seconds * 1000);
  else dt.setTime(dt.getTime() - 10000);
...
(function ($, window, document) {
  ...
  function visible(element) {
    ...
    return (
      ((rect.top >= y1 && rect.top <= y2) ||
        (rect.bottom >= y1 && rect.bottom <= y2)) &&
      ((rect.left >= x1 && rect.left <= x2) ||
        (rect.right >= x1 && rect.right <= x2))
    );
  }
  ...
  function show() {
    ...
    if (!content) {
      var script = element
        .children()
        .filter('script[type="text/lazyload"]')
        .get(0);
      content = $(script).html();
    }
    if (!content) {
  ...
  function clearWatch() {
    ...
    if (element.find(SELECTOR).length === 0) {
      element
        .removeData(SCROLLER_KEY)
        .removeData(DISPLAY_KEY)
        .removeData(WATCH_KEY);
      element
        .unbind("scroll", detect)
        .unbind(EVENT, clearWatch)
        ._unbindShow(detect);
    }
  }
